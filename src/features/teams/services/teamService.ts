...
import { materialStockService } from '@/features/stock/services/materialStockService';
...
export function addTeam(
  ...
  void import('@/lib/supabase/supabaseSyncService').then(({ upsertTeam }) => upsertTeam(team).catch(() => {}));
  ...
  void import('@/lib/supabase/supabaseNotificationService').then(({ pushNotificationToSupabase }) =>
...
export function updateTeam(
  ...
  if (updated) {
    void import('@/lib/supabase/supabaseSyncService').then(({ upsertTeam }) => upsertTeam(updated).catch(() => {}));
    const actor = actorFromUser(currentUser);
    if (actor) {
      logEvent(actor, {
        action: 'TEAM_UPDATED',
