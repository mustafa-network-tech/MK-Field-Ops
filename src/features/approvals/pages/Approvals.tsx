...
import { getJobsForUser } from '@/features/jobs/services/jobScopeService';
import { getTeamsForUser } from '@/features/teams/services/teamScopeService';
import { updateJob } from '@/features/jobs/services/jobService';
import { getProjectDisplayKey } from '@/shared/utils/projectKey';
import { Card } from '@/shared/components/ui/Card';
import type { JobMaterialUsage, MaterialMainType } from '@/shared/types';
...
export function Approvals() {
  ...
  const handleApprove = async (jobId: string) => {
    ...
    const result = await updateJob(user ?? undefined, companyId, jobId, { status: 'approved', approvedBy: user!.id });
    if (result.ok) {
  ...
  const handleReject = async (jobId: string) => {
    setActionError('');
    setSuccessMessage(null);
    const result = await updateJob(user ?? undefined, companyId, jobId, { status: 'rejected', rejectedBy: user!.id });
    if (result.ok) {
